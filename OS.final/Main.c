...
void sys_initialize()
{
	...
	for (i = 0; i < 5; i++)
	{
		...
		case 0:
			...
			strcpy(job.statusCN, "运行"); // 进程状态初始化为就绪态
			strcpy(job.command, "/root/a.sh");

			break;
		case 1:
			// 使用 strcpy() 函数复制字符串到 user_name 字段
			strcpy(job1.user_name, "user1");
			job1.pid = 4421;
			job1.priority = 3;

			// 将时间拼接好后赋值给 job.start
			sprintf(job1.start, "%02d:%02d", hour, minute);

			strcpy(job1.statusCN, "就绪"); // 进程状态初始化为就绪态
			strcpy(job1.command, "/user1/b.sh");
			break;
		case 2:
			// 使用 strcpy() 函数复制字符串到 user_name 字段
			strcpy(job2.user_name, "root");
			job2.pid = 4422;
			job2.priority = 3;

			// 将时间拼接好后赋值给 job.start
			sprintf(job2.start, "%02d:%02d", hour, minute);

			strcpy(job2.statusCN, "运行"); // 进程状态初始化为就绪态
			strcpy(job2.command, "/root/c.sh");
			break;
		case 3:
			// 使用 strcpy() 函数复制字符串到 user_name 字段
			strcpy(job3.user_name, "user2");
			job3.pid = 4423;
			job3.priority = 3;

			// 将时间拼接好后赋值给 job.start
			sprintf(job3.start, "%02d:%02d", hour, minute);

			strcpy(job3.statusCN, "阻塞"); // 进程状态初始化为就绪态
			strcpy(job3.command, "/user2/d.sh");
			break;
		case 4:
			// 使用 strcpy() 函数复制字符串到 user_name 字段
			strcpy(job4.user_name, "root");
			job4.pid = 4424;
			job4.priority = 2;

			// 将时间拼接好后赋值给 job.start
			sprintf(job4.start, "%02d:%02d", hour, minute);

			strcpy(job4.statusCN, "就绪"); // 进程状态初始化为就绪态
			strcpy(job4.command, "/root/e.sh");
			break;
		default:
			break;
		}
	}
	process_insert(memory_list, &process_list, job);
	process_insert(memory_list, &process_list, job1);
	process_insert(memory_list, &process_list, job2);
	process_insert(memory_list, &process_list, job3);
	process_insert(memory_list, &process_list, job4);
	print_log_info("Initializing proccess manage complete...");
...
{
	printf("------------------------进程管理————命令浏览---------------------------\n");
	printf("*                > 创建新的进程 add [-name] [-priority] [-status]     *\n");
	printf("*                > 查 看 进 程  ps aux                                *\n");
	printf("*                > 挂起某个进程 pause [-PID]                          *\n");
	printf("*                > 杀死运行进程 kill [-PID]                           *\n");
	printf("*                > 唤醒某个进程 continue [-PID]                       *\n");
	printf("*                > 运行进程     exec                                  *\n");
	printf("*                > 设置进程优先级 nice [-PID] [-PRIORITY]             *\n");
	printf("*                > 清除屏幕 clear                                     *\n");
	printf("*                > 退出系统 exit                                      *\n");
	printf("*                [-status: 0 - 运行 / 1 - 挂起 / 2 - 阻塞 ]           *\n");
	printf("-----------------------------------------------------------------------\n");
}
...
int main_menu(struct user_node *p)
{
	...
	while (1)
	{
		...
		else if (0 == strcmp(ser_cmd, "add"))
		{
			...
			while (token != NULL)
			{
				...
				case 2: // 状态
					if (0 == atoi(token))
					{
						strcpy(new_job.statusCN, "运行");
					}
					else if (1 == atoi(token))
					{
						strcpy(new_job.statusCN, "挂起");
					}
					else if (2 == atoi(token))
					{
						strcpy(new_job.statusCN, "阻塞");
					}
					break;
				default:
					break;
		...
		else if (0 == strcmp(ser_cmd, "pause"))
		{
			// pause 2311
			char *args = cmd + 6;
			process_pause(&process_list, atoi(args));
		}
		else if (0 == strcmp(ser_cmd, "continue"))
		{
			// continue 2311
			char *args = cmd + 9;
			process_continue(&process_list, atoi(args));
		}
